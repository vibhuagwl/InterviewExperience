...
class Solution {
	...
	public static void getTargetCombination(int[] arr, int position, int currentTarget, List<Integer> result) {

		/**
		 * Base case 1. If currentTarget is reaching to Zero 2. Current Position is
		...
		/**
		 * There are two cases 1. Pick the current value if the current value (i.e
		 * arr[position]) is less than or equal to the currentTarget value then use the
		 * same attribute by passing the same position
		 *
